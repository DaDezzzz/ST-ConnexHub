...
4. 用原来的连接正常聊天。出现中断、长时间无输出或报错后，点 **导出日志**，得到 `connexhub-diagnostics-*.json`。诊断只保留最近一次请求，复现后请在发送下一次请求前导出。
...
- 本机 `localStorage` 独立键仅保存最近一次请求摘要，并限制单条归档大小。每天按本机时间 06:00 清空；若页面关闭、设备休眠或服务停止而错过时刻，会在下次启动、恢复页面或保存日志时补清。无痕模式、清理浏览器数据或更换浏览器/酒馆地址后可能无法恢复。同站点的其他脚本也能访问，勿在共用设备长期保留。
- 开关和日志不写入酒馆账户配置，不上传云端。存储被禁用/写满时退回当前页面内存，面板会提示，请关闭页面前导出。
- 内存中可短暂跟踪少量并发请求，但面板持久化和导出只保留最近开始的一次。多标签页写入是尽力而为，较早请求的迟到快照不会覆盖较新请求。页面重载留下的进行中快照标为「上次记录未完成」，**不是直接认定网络中断**。
